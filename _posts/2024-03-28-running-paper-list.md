...
where $Z(x)$ is a function of $x$ only. And one can rearrange the above equation for $r(x,y)$, and get 
$$r(x, y) = \beta\log \dfrac{\pi_r(y|x)}{\pi_{\text{ref}}(y|x)} + \beta\log Z(x)$$
